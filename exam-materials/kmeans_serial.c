 ...
 * kmeans_serial.c
 * ---------------
 * Serial reference implementation of K-means.
 *
 * Study value:
 * - This is the algorithm that Problem 3 parallelizes.
 * - The MPI version preserves the same conceptual phases:
 *     (1) recompute centroids
 *     (2) reassign points
 *     (3) repeat until assignments stop changing
 * - Understanding the serial data flow makes the collective operations in
 *   the MPI version feel much less arbitrary.
 */
...
typedef struct {
    ...
    int dim;        /* number of features per point */
    float *features;/* flattened feature matrix */
    int *assigns;   /* current cluster assignment per point */
    int *labels;    /* ground-truth labels from dataset */
    int nlabels;    /* count of distinct labels */
} KMData;

typedef struct {
    int nclust;      /* number of clusters */
    int dim;         /* centroid dimensionality */
    float *features; /* centroid coordinates */
    int *counts;     /* points assigned to each cluster */
} KMClust;

/* Load text dataset into contiguous arrays. */
KMData kmdata_load(const char *datafile) {
    ...
    data.dim = tokens_per_line - 2; /* label and colon are not features */
...
/* Allocate centroid storage and zero-initialize it. */
...
/* Write cluster centers as PGM images when feature dimension is square. */
...
int main(int argc, char **argv) {
    ...
    /* Initialize cluster sizes implied by the assignment policy. */
    ...
    while (nchanges > 0 && curiter <= MAXITER) {
        /* Zero centroid accumulators before recomputing means. */
        for (int i = 0; i < nclust * clust.dim; i++) {
            clust.features[i] = 0.0f;
        }

        /* Sum feature vectors into their currently assigned cluster. */
        for (int i = 0; i < data.ndata; i++) {
        ...
        /* Divide sums by cluster counts to get centroids. */
        for (int c = 0; c < nclust; c++) {
            if (clust.counts[c] > 0) {
                for (int d = 0; d < clust.dim; d++) {
                    clust.features[(size_t)c * (size_t)clust.dim + (size_t)d] /=
        ...
        /* Reset counts; they will be recomputed during reassignment. */
        for (int c = 0; c < nclust; c++) clust.counts[c] = 0;

        nchanges = 0;

        /* Reassign every point to the nearest centroid. */
        for (int i = 0; i < data.ndata; i++) {
            int best_clust = 0;
    ...
    /* Build confusion matrix for evaluating how clusters line up with labels. */
